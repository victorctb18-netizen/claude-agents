...
6. **tester** — lista exata de arquivos tocados e comportamento esperado. Falhou → `SendMessage` pro mesmo worker (ele já tem o contexto; spawn novo recomeça do zero) com o erro colado e o motivo provável. Achado do reviewer vai pelo mesmo caminho. Máximo 2 voltas: a 3ª falha é problema de plano, não de execução — pare, releia o erro no root e replaneje ou reporte.
...
- **Contexto**: o pedido original do usuário em 1-2 linhas (a subtarefa sozinha perde o porquê) + o mapa do explorer; não mande redescobrir.
